"""Execute an approved release resource through its real browser confirmation dialog."""
import argparse
import json
...
parser.add_argument('--run', required=True, choices=['mqtt-release', 'mqtt-restore', 'mcp-deploy-version', 'mcp-restore'])
args = parser.parse_args()
root = Path(__file__).resolve().parents[2]
api = Komodo(root)
kind = 'Action' if args.run == 'mcp-deploy-version' else 'Procedure'
label = '运行自动化脚本' if kind == 'Action' else '运行流水线'
procedure = next(p for p in api.call('read/List'+kind+'s', {}) if p['name'] == args.run)
...
with sync_playwright() as p:
    ...
    page.goto('http://127.0.0.1:28793/'+kind.lower()+'s/'+procedure['id'], wait_until='networkidle')
    page.get_by_role('button', name=label, exact=True).click()
    dialog = page.get_by_role('dialog')
    dialog.wait_for()
    dialog.locator('input').fill(args.run)
    with page.expect_response(lambda response: '/execute/Run'+kind in response.url) as submitted:
    ...
    report = {'resource':args.run,'kind':kind,'browser_confirmed':True,'page_errors':errors,'execution':execution}
