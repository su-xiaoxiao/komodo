...
class Bridge:
        ...
        for claim in claims:
            ...
            try:
                if target.exists():
                    ...
                    if recorded.get('status') in STACK_TERMINAL:
                        # A verdict already reported stands, but a 'still executing' record from a
                        # dead process is not true any more: the CLI is gone and only the Docker
                        # daemon knows whether its operation finished.
                        execution = recorded.get('execution') or {}
                        if execution.get('running'):
                            execution = dict(execution, running=False, restarted=True,
                                             note='executor restarted while this operation was in flight; the reported '
                                                  'verdict stands and the container state must be reconciled against docker')
        ...
        for path in sorted((self.home/'requests').glob('*.json')):
            ...
            try:
                ...
                if intent['action'] == 'stack':
                    # The reported verdict is settled once: a timed out or interrupted operation
                    # is never turned back into 'running', and never executed a second time. The
                    # separate execution record may still be refreshed while the background
                    # operation has news (it finished after the timeout verdict was reported).
                    if target.exists():
                        recorded = read(target)
                        if recorded.get('status') in STACK_TERMINAL:
                            live = self.stack_runs.get(identity)
                            execution = recorded.get('execution') or {}
                            # No run in this process means there is nothing to add (startup
                            # reconciliation already settled what an earlier executor left).
                            if live is None:
                                continue
                            # While the background work is unchanged the answer stays byte-identical;
                            # it is refreshed exactly once, when the real outcome lands.
                            if (execution.get('running') == self.busy(live)
                                    and execution.get('outcome') == live.get('outcome')):
                                continue
                ...
                if claim.exists():
                    ...
                    if intent['action'] in ('build-deploy', 'deploy', 'rollback'):
                        ...
                        if any(self.busy(state) for state in self.stack_runs.values()):
                            # Busy means still executing, even if the operator was already told
                            # the operation timed out: that verdict never released the containers.
    ...
    def stack_runner(self, identity, request):
        ...
        if claim.exists():
            ...
            return {'status': 'interrupted', 'stage': 'claimed-without-executor', 'output': [], 'error': STACK_CLAIM_REASON,
                    'started_at': time.time(), 'running': False, 'outcome': None, 'finished_at': None}
        operation, group = request['options']['operation'], request['options']['group']
        # 'status' is what the operator has been told; 'running' is whether this executor still
        # has work in flight. They are deliberately separate records: reporting a timeout does
        # not end the thread, and only 'running' may decide whether the executor is idle.
        state = {'status': 'running', 'stage': 'stack-' + operation, 'output': [], 'error': None,
                 'started_at': time.time(), 'running': True, 'outcome': None, 'finished_at': None,
                 'deadline_guard': False}

        def deadline():
            return state['started_at'] + self.stack_timeout

        def finish(outcome, stage, error=None):
            """Record the real outcome; never rewrite a verdict already reported."""
            state['outcome'] = outcome
            state['running'] = False
            state['finished_at'] = time.time()
            if error is not None:
                state['outcome_error'] = error
            if state['status'] == 'timed_out':
                # The operator was already told 'unknown'; keep that verdict and keep the real
                # outcome next to it (in 'execution') so it can be reconciled, never swallowed.
        ...
        if self.root is None:
            finish('failed', 'error', 'A platform root is required for lifecycle operations')
            return state
        ...
        def run(command):
            ...
            if remaining <= 0:
                # The overall budget is spent: do not start another docker command, and never
                # let the report 'timed_out' imply the background work stopped - it did not.
                state['deadline_guard'] = True
                raise stack_groups.StackError('Operation deadline reached; no further docker command was started '
                                              '(stopping the client does not cancel an operation the Docker daemon '
                                              'already accepted)')
            state['output'].append(' '.join(str(item) for item in command))
            budget = min(self.command_timeout, remaining)
            try:
                result = subprocess.run(command, capture_output=True, text=True, encoding='utf-8',
                                        errors='replace', shell=False, timeout=budget)
            except subprocess.TimeoutExpired:
                raise stack_groups.StackError('docker compose command timed out after {0:.0f}s; the command was stopped '
                                              'and the operation state is unknown'.format(budget))
        ...
        def work():
            try:
                ...
                finish('failed', 'error', str(error)[:500])
    ...
    def answer_stack(self, identity, request, target):
        ...
        if state.get('running') and state['status'] == 'running' and time.time() > state['started_at'] + self.stack_timeout:
            state.update(status='timed_out', stage='timeout', verdict_at=time.time(),
                         error='Lifecycle operation exceeded {0:.0f}s. The reported result is unknown AND the background '
                               'operation is still running; stopping the docker client does not cancel an operation the '
                               'daemon already accepted. Re-check the same entry (operation=status or job=<id>) and '
                               'reconcile against docker before resubmitting'.format(self.stack_timeout))
        execution = {'running': bool(state.get('running')), 'outcome': state.get('outcome'),
                     'finished_at': state.get('finished_at'), 'deadline_guard': bool(state.get('deadline_guard'))}
        if state.get('outcome_error'):
            execution['error'] = state['outcome_error']
        if state['status'] == 'timed_out':
            if execution['running']:
                execution['note'] = ('verdict reported while the background operation was still running; the actual '
                                     'container state must be reconciled against docker before resubmitting')
            else:
                execution['note'] = ('verdict reported at an unknown point; the background operation has since finished '
                                     'and its outcome is recorded here - reconcile it against docker before resubmitting')
    ...
    def busy(self, state):
        """Is this lifecycle operation still executing? Reported verdicts are irrelevant here."""
        if 'running' in state:
            return bool(state['running'])
        return state.get('status') == 'running'

    def active(self):
        # Observation files are never authoritative for stopping an executor, but in-flight
        # lifecycle work is: a run reported as timed_out may still be executing, so idleness is
        # judged on the execution record, never on the verdict the operator was given.
        if any(self.busy(state) for state in self.stack_runs.values()):
            return True
        try:
