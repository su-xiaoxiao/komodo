...
// Downstream presentation-only localization; upstream TSX stays unchanged.
import localizationPlugin from "./localization/transform.mjs";

dotenv.config({ path: ".env.development" });

// https://vitejs.dev/config/
export default defineConfig({
  plugins: [localizationPlugin(), react()],
  server: {
    allowedHosts: process.env.ALLOWED_HOSTS?.split(","),
  },
  resolve: {
    alias: [
      { find: /^mogh_ui$/, replacement: path.resolve(import.meta.dirname, "./src/localization/shared-ui.tsx") },
      { find: /^komodo-shared-ui$/, replacement: path.resolve(import.meta.dirname, "./node_modules/mogh_ui/dist/index.js") },
